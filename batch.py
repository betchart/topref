...
parser.add_option("--data",  dest = "data",     default = False,  action='store_true',  help = 'submit data samples')
parser.add_option("--noask", dest = "noask",    default = False,  action='store_true',  help = 'submit without asking')

options,args = parser.parse_args()
if len(args)!=1 or sum(int(i) for i in [options.doAll,options.justTops,options.justWJets,options.noData,options.data])!=1:
    parser.print_help()
    exit()
...
groupsAll = {80:[ '%s.B.1.jw'%lep,
             ...
             10:['top_s_ph.pu.sf',
                 ...
                 'tbar_t_ph.pu.sf',
                 'tbar_tW_ph.pu.sf'],
             284:['ttj_%s.wGG.pu.sf'%top,'ttj_%s.wQG.pu.sf'%top,'ttj_%s.wQQ.pu.sf'%top,'ttj_%s.wAG.pu.sf'%top,]
             }

groupsTop = {71:['ttj_%s.wGG.pu.sf'%top,'ttj_%s.wQG.pu.sf'%top,'ttj_%s.wQQ.pu.sf'%top,'ttj_%s.wAG.pu.sf'%top,]}
...
groupsNoData = {40:['.'.join([lep,part,'jw']) for part in ['A.1','A.2','B.1','C.1','C.2','C.3','D.1'] for lep in [lep]]}

cmds = []
groups = groupsW if options.justWJets else groupsTop if options.justTops else groupsNoData if (options.noData or options.data) else groupsAll
